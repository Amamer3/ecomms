...
export type DeliveryStatus = "available" | "accepted" | "delivered";
...
export type DeliveryTracking = {
  ...
  vendorName: string;
  orderId: string;
  /** When set (lowercase match), this order appears on the customer `/account` profile. */
...
export const NEARBY_DELIVERY_KM = 3.5;

export const deliveryTrackingSeed: DeliveryTracking[] = [
  {
    id: "R-2031",
    vendorId: "akws-farms",
  ...
  },
  {
    ...
    courierPoint: { lat: 5.665, lng: -0.0093 },
    pickupPoint: { lat: 5.6698, lng: -0.0166 },
    dropoffPoint: { lat: 5.6384, lng: -0.1015 },
  },
];

export function getVendorDeliveries(vendorId: string) {
  return deliveryTrackingSeed.filter((delivery) => delivery.vendorId === vendorId);
}

export function getActiveDeliveries(deliveries = deliveryTrackingSeed) {
...
/** Demo email that unlocks three seeded orders on the customer profile. */
export const CUSTOMER_TRACKER_DEMO_EMAIL = "customer-tracker@randys.local";

const PROFILE_DEMO_ORDER_IDS = new Set(["R-2031", "R-2029", "R-2024"]);

/** Orders for `/account`: match email, or show demo samples so the map is always explorable. */
export function getCustomerProfileOrders(
  email: string,
  displayName: string,
): { orders: DeliveryTracking[]; isDemoSample: boolean } {
  const e = email.trim().toLowerCase();
  const matched = e
    ? deliveryTrackingSeed.filter((d) => d.customerContactEmail?.toLowerCase() === e)
    : [];
  const isDemoSample = matched.length === 0;
  const base = isDemoSample
    ? deliveryTrackingSeed.filter((d) => PROFILE_DEMO_ORDER_IDS.has(d.id))
    : matched;
